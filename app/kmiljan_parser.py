...
def download_modules(courses):
    ...
    groups = {
        x.get_text(): x["value"]
        for x in base_soup.find_all("option", value=re.compile(r"form"))
    }
    ...
    for module in modules:
        ...
        for year in modules[module]:
            ...
            for course in courses_list:
                ctypes = set(
                    [x["course_type"] for x in courses if x["description"] == course]
                )
                courses_dict[course] = {
                    ctype: list(
                        set(
...
def download_courses():
    ...
    rooms = [
        (x["value"], x.get_text())
        for x in base_soup.find_all("option", value=re.compile(r"room"))
    ]
    courses = []
            ...
            for td in row.find_all("td")[1::]:
                ...
                if td.get_text().strip():
                    if not td.has_attr("colspan"):
                        td["colspan"] = 1
                    duration = int(td["colspan"])
                    courses.append(get_course(td, i, time, room_name).__dict__)
...
def fix_disjoint(courses):
    ...
    for i, _ in enumerate(courses):
        ...
        while j < len(courses) and courses[j]["start"] == courses[i]["end"]:
            ...
            if (
                courses[j]["description"] == courses[i]["description"]
                ...
                print(
                    "joining {} for {}".format(
                        courses[i]["description"], courses[i]["groups"]
                    ),
                    file=sys.stderr,
                )
                # then join
